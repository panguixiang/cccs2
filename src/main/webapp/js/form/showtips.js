...
function showtip(msg, required, maxLength, tipWidth) {
	// tips--提示信息
	// required-- validate如果有 required 则是必填项
	// maxLength--输入长度限制
	// tipWidth--提示图层宽度
	...
	if (maxLength) {
		tips = tips + '<font color="red">长度限制：' + maxLength
				+ '字节(一个汉字等于2个字节)</font>&nbsp';
	}
	my_tips.innerHTML = tips;
	my_tips.style.display = "block";
	my_tips.style.width = tipWidth + "px";
	my_tips.style.left = event.clientX + 10 + document.body.scrollLeft+"px";
	my_tips.style.top = event.clientY + 10 + document.body.scrollTop
			+ "px";
