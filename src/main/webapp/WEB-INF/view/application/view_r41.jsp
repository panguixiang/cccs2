...
<head>
<title>在线查看-软件登记事项变更或补充申请表</title>
<STYLE>TABLE {
...
border=0 cellSpacing=0 cellPadding=0 width="100%" align=center>
  ...
  <TR>
    ...
    ?>
      <DIV class=title>软件登记事项变更或补充申请</DIV></TD></TR></TBODY></TABLE>
  ...
  <TR>
    ...
    <TD>
      <TABLE border=1 cellSpacing=0 cellPadding=0 width="100%" frame=void>
        <TBODY>
        <TR height=70>
          <TH style="WIDTH: 100px">软件名称<BR>及版本号</TH>
          <TD style="WIDTH: 530px" class=InputTd><SPAN id=nameSpan 
            class=suporsub><c:out value="${r41.softFullName}"></c:out></SPAN></TD></TR>
        <TR height=30>
          <TH style="WIDTH: 100px">原登记类别</TH>
          <TD style="WIDTH: 530px" class=InputTd>
           <c:if test="${r41.originalRegType=='软件著作权登记'}">⊙</c:if>
		   <c:if test="${r41.originalRegType!='软件著作权登记'}">○</c:if>&nbsp;软件著作权登记&nbsp;
		   <c:if test="${r41.originalRegType=='软件著作权转让合同登记'}">⊙</c:if>
		   <c:if test="${r41.originalRegType!='软件著作权转让合同登记'}">○</c:if>&nbsp;软件著作权转让合同登记&nbsp;
		   <c:if test="${r41.originalRegType=='软件著作权专有许可合同登记'}">⊙</c:if>
		   <c:if test="${r41.originalRegType!='软件著作权专有许可合同登记'}">○</c:if>&nbsp;软件著作权专有许可合同登记&nbsp;
          </TD></TR>
        <TR height=30>
          <TH style="WIDTH: 100px">原登记号</TH>
          <TD style="WIDTH: 530px" class=InputTd><c:out value="${r41.originalRegNumber}"></c:out></TD></TR>
        <TR height=50>
          <TH style="WIDTH: 100px">原变更/补充<BR>登记证明编号</TH>
          <TD style="WIDTH: 530px" 
      class=InputTd><c:out value="${r41.originalChangeNumber}" /></TD></TR></TBODY></TABLE></TD></TR>
  <TR>
    <TD class=FenLeiInfo>
      <DIV>变更或补充信息</DIV></TD>
    <TD>
      <TABLE border=1 cellSpacing=0 cellPadding=0 width="100%" frame=void>
        ...
        <TR height=30>
          ...
          <TD style="WIDTH: 530px" class=InputTd>
          <c:if test="${r41.applyingType=='变更登记'}">⊙</c:if>
		  <c:if test="${r41.applyingType!='变更登记'}">○</c:if>&nbsp;变更登记&nbsp;
		  <c:if test="${r41.applyingType=='补充登记'}">⊙</c:if>
		  <c:if test="${r41.applyingType!='补充登记'}">○</c:if>&nbsp;补充登记		
          </TD></TR>
        <TR>
          <TD width=630 colSpan=2>
            <TABLE border=1 cellSpacing=0 cellPadding=0 width="100%" 
              frame=void><TBODY>
              <TR height=30>
               <c:if test="${r41.applyingType=='变更登记'}">
                <TH>变更事项</TH>
                <TH>变更前内容</TH>
                <TH>变更后内容</TH></TR>
                </c:if>
                <c:if test="${r41.applyingType=='补充登记'}">
                 <th>补充项</th>
                 <th>补充前内容</th>
                 <th>补充后内容</th>
                </c:if>
                <c:forEach items="${infos}" var="info1" varStatus="idx">
              <TR height=70>
                <TD style="LINE-HEIGHT: 16px; FONT-SIZE: 12px" 
                  class=InputTd>${info1.item}</TD>
                <TD style="LINE-HEIGHT: 16px; FONT-SIZE: 12px" 
                  class=InputTd><SPAN id=nameSpan class=suporsub>${info1.before_content}</SPAN></TD>
                <TD style="LINE-HEIGHT: 16px; FONT-SIZE: 12px" 
                  class=InputTd><SPAN id=nameSpan 
                class=suporsub>${info1.after_content}</SPAN></TD></TR>
               </c:forEach> 
             </TBODY></TABLE></TD></TR>
        <TR height=70>
          <TH style="WIDTH: 100px">
            <c:if test="${r41.applyingType=='变更登记'}">变更理由说明</c:if>
		    <c:if test="${r41.applyingType=='补充登记'}">补充理由说明</c:if>       
          </TH>
          <TD style="WIDTH: 530px" 
    class=InputTd><c:out value='${r41.reason}' /></TD></TR></TBODY></TABLE></TD></TR>
    </TBODY></TABLE><BR 
...
cellSpacing=0 cellPadding=0 width="100%" align=center>
  ...
  <TR>
    <TD>&nbsp; </TD>
    <TD 
    style="BORDER-BOTTOM: gray 0px solid; BORDER-LEFT: gray 0px solid; WIDTH: 150px; BORDER-TOP: gray 0px solid;
     BORDER-RIGHT: gray 0px solid"></TD></TR></TBODY></TABLE>
    ...
    <TD colSpan=5>
      <TABLE border=1 cellSpacing=0 cellPadding=0 width="100%" frame=void>
        ...
        <TR>
          ...
          <TD style="WIDTH: 530px" class=InputTd 
            colSpan=3>
           <c:if test="${r41.applyType=='由著作权人办理'}">⊙</c:if>
		   <c:if test="${r41.applyType!='由著作权人办理'}">○</c:if>由著作权人办理&nbsp;&nbsp;
		   <c:if test="${r41.applyType=='由合同登记人办理'}">⊙</c:if>
		   <c:if test="${r41.applyType!='由合同登记人办理'}">○</c:if>由合同登记人办理&nbsp;&nbsp;
		   <c:if test="${r41.applyType=='代理人'}">⊙</c:if>
		   <c:if test="${r41.applyType!='代理人'}">○</c:if>由代理人办理 &nbsp;&nbsp;
      ...
      <TABLE border=1 cellSpacing=0 cellPadding=0 width="100%" frame=void>
        ...
        <TR height=80>
          <TD style="WIDTH: 630px" class=InputTd colSpan=4>
            ...
            id=agentDescU>${r41.agent_desc}</U> </DIV></TD></TR>
        <TR height=50>
          <TH style="WIDTH: 100px">姓名或名称</TH>
