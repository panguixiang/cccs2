...
borderColor=#cccccc cellPadding=0 width="80%" align=center>
  <TBODY>
  <TR>
    <TD style="FONT-SIZE: 14px" align=center><B>撤销或放弃计算机软件登记申请</B> 
      &nbsp;<IMG id=myhelp 
      onmousemove=cpcc_showtip(this.tips,this.valid,this.length,300) 
      onmouseout=cpcc_showtip() src="images/icon_alert.gif" 
      width=18 
    tips="软件著作权登记人或合同登记人，或变更/补充登记人自行撤销原软件著作权登记、合同登记、变更/补充登记时使用此表格。"></TD></TR></TBODY></TABLE>
<DIV id=SoftRegSLZYD>
<TABLE 
...
border=0 cellSpacing=0 cellPadding=0 width="80%" align=center>
  <TBODY>
  <TR>
    <TD class="contentbg small_title" colSpan=2>撤销简况<BR>根据《计算机软件著作权登记办法》二十四条 
      的规定，请求撤销下述软件登记 </TD></TR>
  <TR>
    ...
    <TD class=contentbg><INPUT 
      ...
      type="radio" tips="" <c:if test="${r43_4.originalRegType=='软件著作权登记'}">checked="checked"</c:if> />软件著作权登记 <INPUT 
      onmousemove=cpcc_showtip(this.tips,this.valid,this.length,500) 
      onmouseout=cpcc_showtip() name="originalRegType" value=软件著作权转让合同登记 
      type=radio tips="" <c:if test="${r43_4.originalRegType=='软件著作权转让合同登记'}">checked="checked"</c:if> />软件著作权转让合同登记 
      ...
      type=radio tips="" <c:if test="${r43_4.originalRegType=='软件著作权专有许可合同登记'}">checked="checked"</c:if> />软件著作权专有许可合同登记 </TD></TR>
  <TR>
    ...
    <TD class=contentbg><INPUT 
      ...
      valid="required" value="<c:out value='${r43_4.handleNumber}' />" /></TD></TR>
  <TR>
    ...
    <TD class=contentbg><INPUT 
      ...
      errmsg="登记日期不能为空!" length="40" valid="required" value="<c:out value='${r43_4.handleDate}' />" /> </TD></TR>
  <TR>
    <TD class=contentbg>变更/补充登记证明编号</TD>
    <TD class=contentbg><INPUT 
      onmousemove=cpcc_showtip(this.tips,this.valid,this.length,500) 
      onmouseout=cpcc_showtip() name="originalChangeNumber" tips="" 
      errmsg="变更/补充登记证明编号不能为空!" length="40" valid="" value="<c:out value='${r43_4.originalChangeNumber}' />" /></TD></TR>
  <TR>
    <TD class=contentbg>软件名称及版本号</TD>
    <TD class=contentbg><INPUT style="WIDTH: 448px" 
      onmousemove=cpcc_showtip(this.tips,this.valid,this.length,500) 
      onmouseout=cpcc_showtip() name="softName" size=50 
      tips="填写软件著作权或合同登记证书中记载的软件名称、简称、版本号。" errmsg="软件名称及版本号不能为空!" length="200" 
      valid="required" value="<c:out value='${r43_4.softName}' />" /> </TD></TR>
  <TR>
    ...
    name="reason" tips="" errmsg="撤销登记理由不能为空！" length="200" valid="required"><c:out value='${r43_4.reason}' /></TEXTAREA> 
    </TD></TR>
  <TR>
  ...
  <TR>
    ...
    <TD class=contentbg><INPUT onclick=agent_showOrHide(this) name="applyType" 
      value="由软件登记人办理" type="radio" <c:if test="${r43_4.applyType=='由软件登记人办理'}">checked="checked"</c:if> />由软件登记人办理 
...
function checkGtsqrName(){
  var ids=document.all("hasGtsqr");
  if(ids.checked){
    document.getElementById("gtsqrNameInfo").style.display="block";
	 document.all("gtsqrName").valid="required";
    document.getElementById("gtsqrIdeaInfo").style.display="block";
	 document.all("gtsqrIdea").valid="required";
  }else{
    document.getElementById("gtsqrNameInfo").style.display="none";
	document.all("gtsqrName").valid="no";
    document.getElementById("gtsqrIdeaInfo").style.display="none";
	document.all("gtsqrIdea").valid="no";
  }
}
</SCRIPT>
  ...
  <TR>
    <TD class="contentbg small_title" colSpan=2>
      ...
      <DIV style="FLOAT: right"><INPUT id=hasGtsqr onclick=checkGtsqrName() 
      name="hasGtsqr" value="有共同登记人" type="checkbox" <c:if test="${r43_4.hasGtsqr=='有共同登记人'}">checked="checked"</c:if> /><SPAN 
      style="FONT-SIZE: 13px">有共同登记人&nbsp;&nbsp;</SPAN> </DIV></TD></TR>
  <TR style="DISPLAY: none" id=gtsqrNameInfo>
    <TD class=contentbg>共同登记人名称</TD>
    <TD class=contentbg>
    <TEXTAREA style="BORDER-RIGHT-WIDTH: 2px; WIDTH: 500px; BORDER-TOP-WIDTH: 2px; BORDER-BOTTOM-WIDTH: 2px; BORDER-LEFT-WIDTH: 2px" 
    id="gtsqrNameid" onmousemove=cpcc_showtip(this.tips,this.valid,this.length,350) onmouseout=cpcc_showtip() wrap=hard cols=200 
    rows=3 name="gtsqrName" tips="请准确填写共同登记人名称，并与其他材料保持一致，共同登记人如果有多个，每个名称之间需间隔一个字节。" errmsg="共同登记人名称不能为空！" 
    length="200" valid=""><c:out value='${r43_4.gtsqrName}' /></TEXTAREA> 
    </TD></TR>
  <TR style="DISPLAY: none" id=gtsqrIdeaInfo>
    <TD class=contentbg>共同登记人意见</TD>
    <TD class=contentbg><TEXTAREA style="BORDER-RIGHT-WIDTH: 2px; WIDTH: 500px; BORDER-TOP-WIDTH: 2px; BORDER-BOTTOM-WIDTH: 2px; BORDER-LEFT-WIDTH: 2px"
     id="gtsqrIdeaid" onmousemove=cpcc_showtip(this.tips,this.valid,this.length,350) onmouseout=cpcc_showtip() wrap=hard cols=200 
     rows=3 name="gtsqrIdea" tips="请准确填写共同登记人意见，并与其他材料保持一致，共同登记人意见请简要填写，如，同意撤销此登记。" errmsg="共同登记人意见不能为空！" 
     length="200" valid=""><c:out value='${r43_4.gtsqrIdea}' /></TEXTAREA> 
    </TD></TR>
  <TR>
  ...
  <TR>
    ...
    <TD class=contentbg><INPUT id=sqrpostCode 
      ...
      errmsg="不能为空！|格式为6位数字" length="6" valid="required|isZip" value="<c:out value='${sqr.post_code}' />" /></TD></TR>
  <TR>
 ...
 <TABLE align=center>
  ...
  <TR>
   ...
   <input type="button" value="保存" id="saveId" 
			 class="m_button" onClick="saveForm('${pageContext.request.contextPath}/application/r44/save');"/>
    </TD>
    <TD width=50 style="padding-right:15px;">
    <input type="button" value="提交" id="subId" class="m_button" 
    onClick="subForm('${pageContext.request.contextPath}/application/r44/submit');"/>
    </TD>
   <td>
...
<script language="javascript">
 $(document).ready(function(){
	 ...
	 if(flowNum.length==0) {
	   ...
	   var hasGtsqr = "${r43_4.hasGtsqr}";
	   if(hasGtsqr.length>0) {
		   document.all('hasGtsqr').checked=true;
		   document.getElementById("gtsqrNameInfo").style.display="block";
			 document.all("gtsqrName").valid="required";
		    document.getElementById("gtsqrIdeaInfo").style.display="block";
			 document.all("gtsqrIdea").valid="required";
