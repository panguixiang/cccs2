...
<head>
<title>在线查看-计算机软件著作权转让或专有许可合同登记申请表</title>
<STYLE>TABLE {
...
border=0 cellSpacing=0 cellPadding=0 width="100%" align=center>
  ...
  <TR>
    ...
    style="BORDER-BOTTOM: gray 0px solid; BORDER-LEFT: gray 0px solid; VERTICAL-ALIGN: middle; BORDER-TOP: gray 0px solid; BORDER-RIGHT: gray 0px solid">
      <DIV class=title>计算机软件著作权转让合同和专有许可合同登记申请表</DIV></TD></TR></TBODY></TABLE>
    ...
    <TD>
      <TABLE border=1 cellSpacing=0 cellPadding=0 width="100%" frame=void>
        ...
        <TR height=70>
          ...
          <TD style="WIDTH: 530px" class=InputTd colSpan=3>${r21.softFullName}</TD></TR>
        <TR height=50>
          <TH style="WIDTH: 100px">软件简称</TH>
          <TD style="WIDTH: 530px" class=InputTd colSpan=3>${r21.softForShort}</TD></TR>
        <TR height=30>
          <TH style="WIDTH: 100px">版本号</TH>
          <TD style="WIDTH: 530px" class=InputTd colSpan=3>${r21.edition}</TD></TR>
        <TR height=30>
          <TH style="WIDTH: 117px">软件著作权<BR>登记号</TH>
          <TD style="WIDTH: 248px" class=InputTd>${r21.origRegNumber}</TD>
          <TH style="WIDTH: 110px">变更/补充<BR>登记证明编号</TH>
          <TD style="WIDTH: 255px" 
    class=InputTd>${r21.originalChangeNumber}</TD></TR></TBODY></TABLE></TD></TR>
  <TR><!--<td class="FenLeiInfo"></td>-->
    <TD colSpan=2>
      <TABLE border=1 cellSpacing=0 cellPadding=0 width="100%" frame=void>
        <TBODY>
        <TR height=30>
          <TH style="WIDTH: 134px">合同类型</TH>
  ...
  <TR>
    ...
    <TD>
      ...
      align=center>
        <TBODY>
        <TR height=30>
          <TH style="WIDTH: 370px">姓名或名称</TH>
          <TH style="WIDTH: 60px">国籍</TH>
          <TH style="WIDTH: 200px">身份证明注册地</TH></TR>
          <c:forEach items="${cps1}" var="cp1">
        <TR height=50>
          <TD style="WIDTH: 370px" class=InputTd>${cp1.name}</TD>
          <TD style="WIDTH: 60px" class=InputTd>${cp1.country}</TD>
          <TD style="WIDTH: 200px" class=InputTd>${cp1.province}&nbsp;${cp1.city}</TD></TR>
         </c:forEach>
        </TBODY></TABLE></TD></TR>
  <TR>
    <TD class=FenLeiInfo>
    ...
    <TD>
      ...
      align=center>
        ...
        <TR height=50>
          <TD style="WIDTH: 370px" class=InputTd>${cp2.name}</TD>
          <TD style="WIDTH: 60px" class=InputTd>${cp2.country}</TD>
          <TD style="WIDTH: 200px" class=InputTd>${cp2.province}&nbsp;${cp2.city}</TD></TR>
          </c:forEach>
      </TBODY></TABLE></TD></TR>
  <TR>
    <TD class=FenLeiInfo>
    ...
    <TD>
	 ...
	 <c:when test="${r21.contractType =='转让合同'}">
	  <TBODY>
        <TR height=30>
          <TH width=100>转让权利范围</TH>
          <TD class=InputTd>
            <c:if test="${r21.rightScope!='全部权利'}">○&nbsp;全部权利</c:if>
            <c:if test="${r21.rightScope=='全部权利'}">⊙&nbsp;全部权利</c:if>&nbsp;
            <c:if test="${r21.rightScope!='部分权利'}">○&nbsp;部分权利</c:if>
            <c:if test="${r21.rightScope=='部分权利'}">⊙&nbsp;部分权利</c:if>&nbsp;
          </TD></TR>
        <TR height=70>
          <TH width=100>转让地域范围</TH>
          <TD class=InputTd>${r21.regionScope}&nbsp;&nbsp;${r21.regionScope_}</TD></TR>
        <TR height=30>
          <TH width=100>转让合同生效日期</TH>
          <TD class=InputTd>${r21.validStartDate}
  </TD></TR></TBODY>
	 </c:when>
	 <c:otherwise>
	  <TBODY>
        <TR height=30>
          <TH width=100>许可权利范围</TH>
          <TD class=InputTd>
            <c:if test="${r21.rightScope!='全部权利'}">○&nbsp;全部权利</c:if>
            <c:if test="${r21.rightScope=='全部权利'}">⊙&nbsp;全部权利</c:if>&nbsp;
            <c:if test="${r21.rightScope!='部分权利'}">○&nbsp;部分权利</c:if>
        ...
        <TR height=70>
          <TH width=100>许可地域范围</TH>
          <TD class=InputTd>${r21.regionScope}&nbsp;&nbsp;${r21.regionScope_}</TD></TR>
        <TR height=30>
          <TH width=100>许可权利期限</TH>
          <TD class=InputTd>自${r21.validStartDate}起，至${r21.validEndDate}止。 
  </TD></TR></TBODY>
	 </c:otherwise>
	</c:choose>
  </TABLE></TD></TR>
    ...
    <TD colSpan=5>
      <TABLE border=1 cellSpacing=0 cellPadding=0 width="100%" frame=void>
        ...
        <TR>
          ...
          <TD style="WIDTH: 530px" class=InputTd 
            colSpan=3>
            <c:if test="${r21.applyType!='由转让（许可）方办理'}">○&nbsp;由转让（许可）方办理 </c:if>
			<c:if test="${r21.applyType=='由转让（许可）方办理'}">⊙&nbsp;由转让（许可）方办理 </c:if>&nbsp;&nbsp;
			<c:if test="${r21.applyType!='由受让（被许可）方办理'}">○&nbsp;由受让（被许可）方办理</c:if>
			<c:if test="${r21.applyType=='由受让（被许可）方办理'}">⊙&nbsp;由受让（被许可）方办理</c:if>&nbsp;&nbsp;
			<c:if test="${r21.applyType!='代理人'}">○&nbsp;由代理人办理</c:if>
			<c:if test="${r21.applyType=='代理人'}">⊙&nbsp;由代理人办理</c:if>
             </TD></TR></TBODY></TABLE></TD></TR>
  <TR>
    <TD class=FenLeiInfo>
      ...
      <TABLE border=1 cellSpacing=0 cellPadding=0 width="100%" frame=void>
        ...
        <TR height=80>
          <TD style="WIDTH: 630px" class=InputTd colSpan=4>
            ...
            id=agentDescU>${r21.agent_desc}</U> </DIV></TD></TR>
        <TR height=50>
          <TH style="WIDTH: 100px">姓名或名称</TH>
...
<TABLE border=2 cellSpacing=0 cellPadding=0 width="100%">
  ...
  <TR>
    ...
    <TD class=InputTd>
      ...
      <DIV style="FLOAT: right">一式一份</DIV><BR>
      <DIV style="FLOAT: left">5、软件著作权专有许可合同</DIV>
