	...
	<div class="table_content">
		...
		<div class="app_table">
			<form action=""
				...
				<input type="hidden" name="flow_num" value="${z62.flow_num}">
				<div id="SoftRegSLZYD">
					...
					<td><input name="register_type" value="z11" onclick="changeRegType(1);"
					  type="radio" <c:if test="${z62.register_type=='z11'}">checked="checked"</c:if>>作品著作权登记&nbsp;&nbsp;
					<input name="register_type" value="z21" onclick="changeRegType(2);"
					<c:if test="${z62.register_type=='z21'}">checked="checked"</c:if> type="radio">著作权合同备案&nbsp;&nbsp;
					<input name="register_type" value="z31" onclick="changeRegType(3);"
					<c:if test="${z62.register_type=='z31'}">checked="checked"</c:if> type="radio">与著作权相关权利事项登记&nbsp;&nbsp;
					<br> <input name="register_type" value="z41" onclick="changeRegType(4);"
					<c:if test="${z62.register_type=='z41'}">checked="checked"</c:if> type="radio">重印国外期刊合同登记&nbsp;&nbsp;
                    <input name="register_type" value="z51" onclick="changeRegType(5);"
                    <c:if test="${z62.register_type=='z51'}">checked="checked"</c:if>  type="radio">著作权质权登记&nbsp;&nbsp;
								...
								<td>
									<div style="float: left;">
										<input class="focus" name="opus_name" size="40" tips="请填写作品名称"
											...
											onmouseout="hidden_tips()" type="text" value="<c:out value='${z62.opus_name}' />" >
									</div>
									<div id="z21_opus" style="display: none; float: left;">
										&nbsp;&nbsp;
									作品数量: <input name="more_opus" id="more_opusId" value="<c:out value='${z62.more_opus}' />" size="10"
						...
						<tbody id="z31_type" style="display: none;">
							<tr>
								<td width="150">类别</td>
								<td>
								<input name="right_type" value="录音制品" type="radio"
								 <c:if test="${z62.right_type=='录音制品'}">checked="checked"</c:if> >录音制品 </input>
								 <input name="right_type" value="录像制品" type="radio" <c:if test="${z62.right_type=='录像制品'}">checked="checked"</c:if>>录像制品 </input> <input
									name="right_type" value="表演" type="radio" <c:if test="${z62.right_type=='表演'}">checked="checked"</c:if>>表演</input>
								<input name="right_type" value="版式设计" type="radio" <c:if test="${z62.right_type=='版式设计'}">checked="checked"</c:if>>版式设计</input> 
								<input name="right_type" value="广播、电视" type="radio" <c:if test="${z62.right_type=='广播、电视'}">checked="checked"</c:if>>广播、电视 </input>
								</td>
							</tr>
						</tbody>
						...
						<tbody id="z41_type" style="display: none;">
							<tr>
								...
								<td><input class="inputtext" name="opus_en_name" id="opusEngNameId" size="50"
									...
									onmouseout="hidden_tips()" type="text" value="<c:out value='${z62.opus_en_name}' />" >
								</td>
							</tr>
						...
						<tbody>
							<tr>
								...
								<td><input class="inputtext" name="original_reg_num"
									...
									onmousemove="show_tips(this, event,350)"
									onmouseout="hidden_tips()" type="text" value="<c:out value='${z62.original_reg_num}' />" >
								</td>
							</tr>
							<tr>
								...
								<input type="text" name="register_date"
									...
									onmousemove="show_tips(this, event,350)"
									onmouseout="hidden_tips()" onclick="fPopCalendar(this, this);"  readonly value="<c:out value='${z62.register_date}' />" >
								</td>
							</tr>
							<tr>
							...
							<tr>
								...
								<td>撤销类型</td>
								<td><input name="rev_type" value="申请人申请撤销" onclick="show_evidence(1)" 
								<c:if test="${z62.rev_type=='申请人申请撤销'}">checked="checked"</c:if> type="radio">申请人申请撤销 
								<input name="rev_type" value="第三方申请撤销" onclick="show_evidence(2)"
										type="radio" <c:if test="${z62.rev_type=='第三方申请撤销'}">checked="checked"</c:if> >第三方申请撤销 <input name="rev_type" value="其他"
											onclick="show_evidence(1)" type="radio" <c:if test="${z62.rev_type=='其他'}">checked="checked"</c:if>>其他 
								</td>
							</tr>
						...
						<tbody id="revevidence" style="display: none;">
							<tr>
								<td>撤销依据</td>
								<td><input name="evidence" value="生效判决" 
									type="radio" <c:if test="${z62.evidence=='生效判决'}">checked="checked"</c:if> >生效判决 
							<input name="evidence" value="仲裁决定" type="radio"
							 <c:if test="${z62.evidence=='仲裁决定'}">checked="checked"</c:if>>仲裁决定 
							<input name="evidence" value="著作权行政处罚决定" type="radio" <c:if test="${z62.evidence=='著作权行政处罚决定'}">checked="checked"</c:if>>著作权行政处罚决定 
								</td>
							</tr>
						</tbody>
							<tr>
								...
								<td><textarea class="focus" name="reason" wrap="hard"
										...
										onmousemove="show_tips(this, event,350)"
										onmouseout="hidden_tips()"><c:out value="${z62.reason}"></c:out></textarea></td>
							</tr>
							<tr>
						...
						<tbody id="apply1" style="">
							<tr>
								...
								type="radio" <c:if test="${z62.apply_type=='由著作权人申请'}">checked="checked"</c:if> >由著作权人申请
							<input name="apply_type1" value="代理人" onclick="agent_showOrHide(this)"
							type="radio" <c:if test="${z62.apply_type=='代理人'}">checked="checked"</c:if>>由代理人申请 
								</td>
							</tr>
						...
						<tbody id="apply2" style="display: none;">
							<tr>
								...
								<td><input name="apply_type2" value="由转让方或许可方申请"
									checked="checked" type="radio" <c:if test="${z62.apply_type=='由转让方或许可方申请'}">checked="checked"</c:if>>由转让方或许可方申请 
							<input name="apply_type2" value="由受让方或被许可方申请" 
							type="radio" <c:if test="${z62.apply_type=='由受让方或被许可方申请'}">checked="checked"</c:if>>由受让方或被许可方申请
					<input name="agent2" value="ok" onclick="s_h_agent(this)"
					 type="checkbox" <c:if test="${z62.agent=='ok'}">checked="checked"</c:if>>由代理人申请 
								</td>
							</tr>
						...
						<tbody id="apply3" style="display: none;">
							<tr>
								...
								<td><input name="apply_type3" value="由权利人申请" onclick="agent_showOrHide(this)" 
									 type="radio" <c:if test="${z62.apply_type=='由权利人申请'}">checked="checked"</c:if> >由权利人申请
						...
						<tbody id="apply4" style="display: none;">
							<tr>
								...
								<td><input name="apply_type4" value="由转让方或许可方申请"
									 type="radio" <c:if test="${z62.apply_type=='由转让方或许可方申请'}">checked="checked"</c:if>>由转让方或许可方申请 
								<input name="apply_type4" value="由受让方或被许可方申请" 
								 type="radio" <c:if test="${z62.apply_type=='由受让方或被许可方申请'}">checked="checked"</c:if>>由受让方或被许可方申请
		<input name="agent4" value="ok" onclick="s_h_agent(this)" type="checkbox" <c:if test="${z62.agent=='ok'}">checked="checked"</c:if>>由代理人申请 
								</td>
							</tr>
                  ...
                  <tbody id="apply5" style="display: none;">
					<tr>
						...
						<td><input name="apply_type5" value="由出质人办理"
							 type="radio" <c:if test="${z62.apply_type=='由出质人办理'}">checked="checked"</c:if>>由出质人办理 
					<input name="apply_type5" value="由质权人办理" type="radio" <c:if test="${z62.apply_type=='由质权人办理'}">checked="checked"</c:if>>由质权人办理
						...
						<tbody>
							...
							<tr>
								...
								<td><input id="sqrphone" name="sqrphone"
									title="格式如010-8888888" size="35" value="<c:out value='${sqr.phone}' />"
							...
							<tr>
								...
								<td><input id="sqrmobile" name="sqrmobile" size="35"
								  ...
								  tips="请照正确的格式" length="30" errmsg="手机号码不能为空！" valid="required"
									onmousemove="show_tips(this, event,300)"
									onmouseout="hidden_tips()">
								</td>
							</tr>
							...
							<tr>
								<td colspan="2">
									&nbsp;&nbsp;&nbsp;&nbsp;申请人委托下述代理人办理登记事宜，具体委托事项如下：<br>
										...
										name="agent_desc" size="60" value="<c:out value='${z62.agent_desc}' />"
...
<TABLE align=center>
  ...
  <TR>
   ...
   <input type="button" value="保存" id="saveId" 
			 class="m_button" onClick="saveForm('${pageContext.request.contextPath}/application/z62/update/save');"/>
    </TD>
    <TD width=50>
    <input type="button" value="提交" id="subId" class="m_button" 
       onClick="subForm('${pageContext.request.contextPath}/application/z62/update/submit');"/>
    </TD>
   <td>
...
$(document).ready(function(){
	var registerType = "${z62.register_type}";
	if(registerType=='z21') {
		$("#nametype").html("涉及作品名称");
		$("#numname").html("原备案号");
		$("#datename").html("原备案日期");
