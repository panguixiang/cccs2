...
<title>在线查看-作品著作权和合同变更或补充登记申请表</title>
<STYLE>TABLE {
...
border=0 cellSpacing=0 cellPadding=0 width="100%" align=center>
  ...
  <TR>
    ...
    >
      <DIV class=title>作品著作权和合同变更或补充登记申请表</DIV></TD></TR></TBODY></TABLE>
	...
	<c:if test="${z60.right_type=='广播、电视'}">⊙</c:if><c:if test="${z60.right_type!='广播、电视'}">○</c:if>&nbsp;广播、电视
	  ...
	  <tr>
	   <th width="100">原受理号</th>
		<td class="InputTd">${z60.orig_admissible_num}</td>
	  </tr>
	  <tr>
	    <th width="100">
    ...
    <TD>
      <TABLE border=1 cellSpacing=0 cellPadding=0 width="100%" frame=void>
        ...
        <TR>
          ...
          <TD class=InputTd>
   <c:if test="${z60.change_type=='变更登记'}">⊙</c:if><c:if test="${z60.change_type!='变更登记'}">○</c:if>&nbsp;变更登记
   <c:if test="${z60.change_type=='补充登记'}">⊙</c:if><c:if test="${z60.change_type!='补充登记'}">○</c:if>&nbsp;补充登记					        
         </TD></TR>
        <TR>
          <TD colSpan=2>
            <TABLE border=1 cellSpacing=0 cellPadding=0 width="100%" 
              frame=void><TBODY>
              <TR>
              <c:if test="${z60.change_type=='变更登记'}">
                <TH width=100>变更事项</TH>
                <TH>变更前内容</TH>
                <TH>变更后内容</TH></TR>
               </c:if> 
              <c:if test="${z60.change_type=='补充登记'}">  
                <TH width=100>补充项</TH>
                <TH>补充前内容</TH>
                <TH>补充后内容</TH></TR>
              </c:if>
              ...
              <TR>
                <TH style="WIDTH: 100px">${info1.item}</TH>
                <TD class=InputTd>${info1.before_content}</TD>
                <TD class=InputTd>${info1.after_content}</TD>
                </TR>
               </c:forEach>
                </TBODY></TABLE></TD></TR>
        <TR>
          <TH>
            <c:if test="${z60.change_type=='变更登记'}">变更理由说明</c:if>
			 <c:if test="${z60.change_type=='补充登记'}">补充理由说明</c:if>
          </TH>
...
<TABLE border=2 cellSpacing=0 cellPadding=0 width="100%">
  ...
  <TR>
    ...
    <TD class=InputTd>
      ...
      <DIV style="FLOAT: right">&nbsp;&nbsp;份/&nbsp;&nbsp;页</DIV><BR>
      <DIV style="FLOAT: left">3、变更或补充事项证明材料</DIV>
      <DIV style="FLOAT: right">&nbsp;&nbsp;份/&nbsp;&nbsp;页</DIV><BR>
