...
<head>
<title>在线查看-撤销出版境外音像制品合同登记申请表</title>
<STYLE>TABLE {
...
border=0 cellSpacing=0 cellPadding=0 width="100%" align=center>
  ...
  <TR>
    ...
    style="BORDER-BOTTOM: gray 0px solid; BORDER-LEFT: gray 0px solid; VERTICAL-ALIGN: middle; BORDER-TOP: gray 0px solid; BORDER-RIGHT: gray 0px solid">
      <DIV class=title>撤销出版境外音像制品合同登记申请表</DIV></TD></TR></TBODY></TABLE>
    ...
    <TD>
      <TABLE border=1 cellSpacing=0 cellPadding=0 width="100%" frame=void>
        ...
        <TR>
          ...
          <TD class=InputTd>
           <c:if test="${z73.register_type!='录音制品'}">○&nbsp;录音制品</c:if>
		   <c:if test="${z73.register_type=='录音制品'}">⊙&nbsp;录音制品</c:if> 
		   <c:if test="${z73.register_type!='影视作品'}">○&nbsp;影视作品</c:if>
		   <c:if test="${z73.register_type=='影视作品'}">⊙&nbsp;影视作品</c:if>
          </TD></TR>
        <TR>
          <TH>名&nbsp;&nbsp;&nbsp;&nbsp;称 </TH>
          <TD class=InputTd colSpan=3>
            ...
            width="100%">
              ...
              <TR>
                ...
                vAlign=bottom><c:out value='${z73.cn_opus_name}' /></TD></TR>
              <TR>
                <TD 
                style="BORDER-RIGHT-WIDTH: 0px; BORDER-TOP-WIDTH: 0px; BORDER-BOTTOM-WIDTH: 0px; BORDER-LEFT-WIDTH: 0px" 
                ...
                <TD 
                style="BORDER-RIGHT-WIDTH: 0px; WIDTH: 90%; BORDER-TOP-WIDTH: 0px; BORDER-BOTTOM-WIDTH: 0px; BORDER-LEFT-WIDTH: 0px" 
                vAlign=top><c:out value='${z73.en_opus_name}' /></TD></TR></TBODY></TABLE></TD></TR>
        <TR>
          <TH>出版类型</TH>
          <TD class=InputTd colSpan=3><c:out value='${z73.publish_type}' /></TD></TR>
        <TR>
          <TH>原登记号</TH>
          <TD class=InputTd colSpan=3><c:out value='${z73.original_reg_num}' /></TD></TR>
        <TR>
          <TH>原登记日期</TH>
          <TD class=InputTd colSpan=3><c:out value='${z73.register_date}' /></TD></TR>
        <TR>
          <TH>出版单位名称</TH>
          <TD class=InputTd colSpan=3><c:out value='${z73.publis_comp_name}' /></TD></TR>
        <TR>
          <TD class=FenLeiInfo rowSpan=3>
            <DIV>撤销简况</DIV></TD></TR>
        <TR>
          <TH>撤销类型</TH>
          <TD class=InputTd>
            <c:if test="${z73.rev_type!='原登记申请人申请撤销'}">○&nbsp;原登记申请人申请撤销</c:if>
		    <c:if test="${z73.rev_type=='原登记申请人申请撤销'}">⊙&nbsp;原登记申请人申请撤销</c:if> 
		    <c:if test="${z73.rev_type!='其他'}">○&nbsp;其他</c:if>
		    <c:if test="${z73.rev_type=='其他'}">⊙&nbsp;其他</c:if>
         </TD></TR>
        <TR>
          <TH>撤销登记理由</TH>
          <TD class=InputTd><c:out value='${z73.rev_desc}' />
    ...
    <TD colSpan=2>
      <TABLE border=1 cellSpacing=0 cellPadding=0 width="100%" frame=void>
        ...
        <TR>
          ...
          <c:if test="${z73.apply_type!='原出版单位申请'}">○&nbsp;原出版单位申请</c:if>
		  <c:if test="${z73.apply_type=='原出版单位申请'}">⊙&nbsp;原出版单位申请</c:if>
		  <c:if test="${z73.apply_type!='代理人'}">○&nbsp;由代理人申请</c:if>
		  <c:if test="${z73.apply_type=='代理人'}">⊙&nbsp;由代理人申请</c:if>
      ...
      <TABLE border=1 cellSpacing=0 cellPadding=0 width="100%" frame=void>
        ...
        <TR height=80>
          <TD style="WIDTH: 630px" class=InputTd colSpan=4>
            ...
            id=agentDescU>${z73.agent_desc}</U></DIV></TD></TR>
        <TR height=50>
          <TH style="WIDTH: 100px">姓名或名称</TH>
...
<TABLE border=2 cellSpacing=0 cellPadding=0 width="100%">
  ...
  <TR>
    <TH>证明文件</TH>
    <TD class=InputTd>
      <DIV style="FLOAT: left">1、原合同登记批复（白联和黄联）</DIV>
      <DIV style="FLOAT: right">&nbsp;&nbsp;份/&nbsp;&nbsp;页</DIV><BR>
      <DIV style="FLOAT: left">2、其他需要提交的证明材料</DIV>
