...
<head>
<title>在线查看-撤回作品著作权和合同登记申请表</title>
<STYLE>TABLE {
...
border=0 cellSpacing=0 cellPadding=0 width="100%" align=center>
  ...
  <TR>
    ...
    ?>
      <DIV class=title>撤回作品著作权和合同登记申请表</DIV></TD></TR></TBODY></TABLE>
    ...
    <TD>
      <TABLE border=1 cellSpacing=0 cellPadding=0 width="100%" frame=void>
        ...
        <TR>
          ...
          <TD class=InputTd>
          <c:if test="${z60.register_type=='z11'}">⊙</c:if>
		<c:if test="${z60.register_type!='z11'}">○</c:if>&nbsp;作品著作权登记&nbsp;

		<c:if test="${z60.register_type=='z21'}">⊙</c:if>
		<c:if test="${z60.register_type!='z21'}">○</c:if>&nbsp;著作权合同备案&nbsp;

		<c:if test="${z60.register_type=='z31'}">⊙</c:if>
		<c:if test="${z60.register_type!='z31'}">○</c:if>&nbsp;与著作权相关权利事项登记&nbsp;

		<br> <c:if test="${z60.register_type=='z41'}">⊙</c:if><c:if
					test="${z60.register_type!='z41'}">○</c:if>&nbsp;重印国外期刊合同登记&nbsp;
		<c:if test="${z60.register_type=='z51'}">⊙</c:if><c:if
					test="${z60.register_type!='z51'}">○</c:if>&nbsp;著作权质权登记&nbsp;	
          </TD></TR>
        <TR>
          <TH width=100>
          <c:if test="${z60.register_type=='z11'}">作品名称</c:if>
		 <c:if test="${z60.register_type=='z21'}">涉及作品名称</c:if>
         <c:if test="${z60.register_type=='z31'}">名称</c:if> <c:if
				test="${z60.register_type=='z41'}">期刊中文名称</c:if>
		<c:if test="${z60.register_type=='z51'}">涉及作品名称</c:if>
         ...
         <tr>
			<th width=100>类别</th>
			<td class="InputTd"><c:if test="${z60.right_type=='录音制品'}">⊙</c:if>
				<c:if test="${z60.right_type!='录音制品'}">○</c:if>&nbsp;录音制品 <c:if
					test="${z60.right_type=='录像制品'}">⊙</c:if>
				<c:if test="${z60.right_type!='录像制品'}">○</c:if>&nbsp;录像制品 <c:if
					test="${z60.right_type=='表演'}">⊙</c:if>
				<c:if test="${z60.right_type!='表演'}">○</c:if>&nbsp;表演 <c:if
					test="${z60.right_type=='版式设计'}">⊙</c:if>
				<c:if test="${z60.right_type!='版式设计'}">○</c:if>&nbsp;版式设计 <c:if
					test="${z60.right_type=='广播、电视'}">⊙</c:if>
				<c:if test="${z60.right_type!='广播、电视'}">○</c:if>&nbsp;广播、电视</td>
				</tr>
			</c:if>
        ...
        <TR>
          <TH width=100>原受理号</TH>
          <TD class=InputTd>${z60.orig_admissible_num}</TD></TR>
        <TR>
          <TH>原受理日期</TH>
          <TD class=InputTd>${z60.register_date}</TD></TR>
          </TBODY></TABLE></TD></TR>
  <TR>
    <TD class=FenLeiInfo>
      <DIV>撤回简况</DIV></TD>
    <TD>
      <TABLE border=1 cellSpacing=0 cellPadding=0 width="100%" frame=void>
        ...
        <TR>
          <TH width=100>
          <c:choose>
          <c:when test="${z60.register_type=='z21'}">撤回备案理由
          </c:when>
          <c:otherwise>撤回登记理由</c:otherwise>
          </c:choose>
           </TH>
          <TD class=InputTd>${z60.reason}</TD></TR></TBODY></TABLE></TD></TR></TBODY></TABLE>
