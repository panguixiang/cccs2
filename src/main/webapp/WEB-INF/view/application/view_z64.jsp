...
border=0 cellSpacing=0 cellPadding=0 width="100%" align=center>
  ...
  <TR>
    ...
    >
      <DIV class=title>补发或更换登记证书申请表</DIV></TD></TR></TBODY></TABLE>
  ...
  <TR>
    ...
    <TD>
      <TABLE border=1 cellSpacing=0 cellPadding=0 width="100%" frame=void>
        <TBODY>
        <TR>
          <TH width=130>原登记类型</TH>
          <TD class=InputTd>
          <c:if test="${z60.register_type=='z11'}">⊙</c:if><c:if test="${z60.register_type!='z11'}">○</c:if>&nbsp;作品著作权登记&nbsp;&nbsp;
					<c:if test="${z60.register_type=='z21'}">⊙</c:if><c:if test="${z60.register_type!='z21'}">○</c:if>&nbsp;著作权合同备案&nbsp;&nbsp;<br>
					<c:if test="${z60.register_type=='z31'}">⊙</c:if><c:if test="${z60.register_type!='z31'}">○</c:if>&nbsp;与著作权相关权利事项登记&nbsp;&nbsp;
					<c:if test="${z60.register_type=='z41'}">⊙</c:if><c:if test="${z60.register_type!='z41'}">○</c:if>&nbsp;重印国外期刊合同登记&nbsp;&nbsp;<br>
		...
		<c:if test="${z60.register_type=='z51'}">涉及作品名称</c:if>       
          ...
          <c:if test="${z60.register_type=='z21'}">
                            作品数量:${z60.more_opus}</c:if> </TD>
          </TR>
		...
		<c:if test="${z60.register_type=='z51'}">原登记号</c:if>
          ...
          <TD class=InputTd>${z60.original_reg_num}</TD></TR>
        <TR>
          <TH>
  ...
  <TR>
    ...
    <TD>
      ...
      height="100%">
        <TBODY>
        <TR>
          <TH width=130>补发或更换证书类型</TH>
          <TD class=InputTd>
           <c:choose>
           <c:when test="${z60.register_type=='z51'}">
		     <c:if test="${z60.change_type=='著作权质权登记证书'}">⊙</c:if>
		     <c:if test="${z60.change_type!='著作权质权登记证书'}">○</c:if>&nbsp;著作权质权登记证书
			 <c:if test="${z60.change_type=='变更或补充证明'}">⊙</c:if>
			 <c:if test="${z60.change_type!='变更或补充证明'}">○</c:if>&nbsp;变更或补充证明
           </c:when>
           <c:otherwise>
           <c:if test="${z60.change_type=='登记证书'}">⊙</c:if>
           <c:if test="${z60.change_type!='登记证书'}">○</c:if>&nbsp;登记证书
		  <c:if test="${z60.change_type=='变更或补充证明'}">⊙</c:if>
		  <c:if test="${z60.change_type!='变更或补充证明'}">○</c:if>&nbsp;变更或补充证明
           </c:otherwise>
           </c:choose>
      ...
      <TABLE border=1 cellSpacing=0 cellPadding=0 width="100%" frame=void>
        <TBODY>
         <c:forEach items="${parts}" var="part">
          ...
          <TD style="WIDTH: 371px" class=InputTd>${part.name}</TD>
          <TH style="WIDTH: 50px">电话</TH>
          <TD style="WIDTH: 90px" class=InputTd>${part.phone}</TD></TR>
		  </c:forEach>
        <TR height=50>
          <TH style="WIDTH: 100px">姓名或名称</TH>
