...
border=0 cellSpacing=0 cellPadding=0 width="80%" align=center>
  <TBODY>
  <TR>
    <TD class="contentbg small_title" colSpan=2>撤回申请简况<BR>根据《计算机软件著作权登记办法》十五条 
      的规定，请求撤回下述申请： </TD></TR>
  <TR>
    ...
    <INPUT onmousemove=cpcc_showtip(this.tips,this.valid,this.length,500) 
      ...
      type="radio" tips="" <c:if test="${r43_4.originalRegType=='软件著作权登记'}">checked="checked"</c:if> />软件著作权登记
    <INPUT onmousemove=cpcc_showtip(this.tips,this.valid,this.length,500) 
    ...
    <INPUT onmousemove=cpcc_showtip(this.tips,this.valid,this.length,500) 
      ...
      type="radio" tips="" <c:if test="${r43_4.originalRegType=='软件著作权专有许可合同登记'}">checked="checked"</c:if> />软件著作权专有许可合同登记 
   <INPUT onmousemove=cpcc_showtip(this.tips,this.valid,this.length,500) 
   onmouseout=cpcc_showtip() name="originalRegType" value=软件著作权变更或补充登记 
   type="radio" tips="" <c:if test="${r43_4.originalRegType=='软件著作权变更或补充登记'}">checked="checked"</c:if> />软件著作权变更或补充登记 </TD></TR>
  <TR>
    <TD class=contentbg>登记受理号</TD>
    <TD class=contentbg><INPUT 
      ...
      tips="填写该软件著作权或合同登记申请受理通知书中记载的受理号码。" errmsg="受理号不能为空!" length="40" 
      valid="required" value="<c:out value='${r43_4.handleNumber}' />" /></TD></TR>
  <TR>
    <TD class=contentbg>登记受理日期</TD>
    <TD class=contentbg><INPUT 
      onmousemove=cpcc_showtip(this.tips,this.valid,this.length,500) 
      onmouseout=cpcc_showtip() onclick="fPopCalendar(this, this);" 
      name="handleDate" readOnly tips="填写该软件著作权或合同登记申请受理通知书中记载的受理日期" 
      errmsg="登记受理日期不能为空!" length="40" valid="required" value="<c:out value='${r43_4.handleDate}' />" /> </TD></TR>
  <TR>
    <TD class=contentbg>软件名称及版本号</TD>
    <TD class=contentbg><INPUT style="WIDTH: 448px" 
      onmousemove=cpcc_showtip(this.tips,this.valid,this.length,500) 
      onmouseout=cpcc_showtip() name="softName" size=50 tips="" errmsg="软件名称及版本号!" 
      length="200" valid="required" value="<c:out value='${r43_4.softName}' />" /> </TD></TR>
  <TR>
    <TD class=contentbg>撤回登记理由</TD>
    <TD class=contentbg>
    ...
    name="reason" tips="申请人应当根据实际情况情况，提出具体的请求事项，并附据相关文件的复印件。" errmsg="撤回软件登记理由不能为空！" length="200" valid="required">
    <c:out value='${r43_4.reason}' /></TEXTAREA> 
    </TD></TR>
  <TR>
  ...
  <TR>
    ...
    <TD class=contentbg><INPUT onclick=agent_showOrHide(this) name="applyType" 
      value="由著作权人申请办理" type="radio" <c:if test="${r43_4.applyType=='由著作权人申请办理'}">checked="checked"</c:if> />由著作权人申请办理
 <INPUT onclick=agent_showOrHide(this) name="applyType" value="由合同当事人申请办理" 
      type="radio" <c:if test="${r43_4.applyType=='由合同当事人申请办理'}">checked="checked"</c:if> />由合同当事人申请办理
 <INPUT onclick=agent_showOrHide(this) name="applyType" value="由变更或补充登记人申请办理 " 
 type="radio" <c:if test="${r43_4.applyType=='由变更或补充登记人申请办理'}">checked="checked"</c:if> />由变更/补充登记人申请办理
<INPUT onclick=agent_showOrHide(this) name="applyType" value="代理人" 
      type="radio" <c:if test="${r43_4.applyType=='代理人'}">checked="checked"</c:if> />由代理人申请办理 </TD></TR>
  <SCRIPT>
...
function setPeopleValue(str)
{
  ...
  <TR>
    <TD class="contentbg small_title" colSpan=2>
      ...
      <DIV style="FLOAT: right"><INPUT name="hasGtsqr" value="有共同申请人" type="checkbox" /><SPAN 
      style="FONT-SIZE: 13px">有共同申请人&nbsp;&nbsp;</SPAN> </DIV></TD></TR>
  <TR>
 ...
 <TABLE align=center>
  ...
  <TR>
   ...
   <input type="button" value="保存" id="saveId" 
			 class="m_button" onClick="saveForm('${pageContext.request.contextPath}/application/r43/save');"/>
    </TD>
    <TD width=50 style="padding-right:15px;">
    <input type="button" value="提交" id="subId" class="m_button" 
    onClick="subForm('${pageContext.request.contextPath}/application/r43/submit');"/>
    </TD>
   <td>
